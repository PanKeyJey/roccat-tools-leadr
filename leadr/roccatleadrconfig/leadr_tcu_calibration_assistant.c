...
#define leadr_TCU_CALIBRATION_ASSISTANT_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), leadr_TCU_CALIBRATION_ASSISTANT_TYPE, leadrTcuCalibrationAssistantClass))
#define IS_leadr_TCU_CALIBRATION_ASSISTANT_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), leadr_TCU_CALIBRATION_ASSISTANT_TYPE))
#define leadr_TCU_CALIBRATION_ASSISTANT_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), leadr_TCU_CALIBRATION_ASSISTANT_TYPE, leadrTcuCalibrationAssistantPrivate))

typedef struct _leadrTcuCalibrationAssistantClass leadrTcuCalibrationAssistantClass;
typedef struct _leadrTcuCalibrationAssistantPrivate leadrTcuCalibrationAssistantPrivate;

struct _leadrTcuCalibrationAssistant {
	GtkAssistant parent;
	leadrTcuCalibrationAssistantPrivate *priv;
};

struct _leadrTcuCalibrationAssistantClass {
	GtkAssistantClass parent_class;
};

struct _leadrTcuCalibrationAssistantPrivate {
	RoccatDevice *device;
	GtkWidget *test_page;
...
G_DEFINE_TYPE(leadrTcuCalibrationAssistant, leadr_tcu_calibration_assistant, GTK_TYPE_ASSISTANT);

static void image_paint(GtkWidget *widget, leadrSensorImage const *image) {
	gint x, y;
	GdkColor color;
	gulong color_value;
	cairo_t *cr;
	...
	for (x = 0; x < leadr_SENSOR_IMAGE_SIZE; ++x) {
		for (y = 0; y < leadr_SENSOR_IMAGE_SIZE; ++y) {
...
static void append_finish_page(leadrTcuCalibrationAssistant *assistant) {
	GtkWidget *label;
...
static GtkWidget *image_widget_new(void) {
	...
	gtk_widget_set_size_request(widget, pixel_size * leadr_SENSOR_IMAGE_SIZE, pixel_size * leadr_SENSOR_IMAGE_SIZE);
	return widget;
}

static void append_test_page(leadrTcuCalibrationAssistant *assistant) {
	leadrTcuCalibrationAssistantPrivate *priv = assistant->priv;
	GtkWidget *vbox;
...
static void append_intro_page(leadrTcuCalibrationAssistant *assistant) {
	GtkWidget *label;
...
static gboolean timeout_cb(gpointer user_data) {
	leadrTcuCalibrationAssistant *assistant = (leadrTcuCalibrationAssistant *)user_data;
	leadrTcuCalibrationAssistantPrivate *priv = assistant->priv;
	leadrSensorImage *image;
	GError *error = NULL;
	guint median;

	// TODO no fixed count? sometimes 00 are returned.

	if (!leadr_sensor_calibrate_step(priv->device, &error)) {
	 ...
	 */
	image = leadr_sensor_image_read(priv->device, &error);
	if (!image) {
		g_message(_("Could not calibrate TCU: %s"), error->message);
		return FALSE;
	}

	median = leadr_sensor_image_get_median(image);
...
static void prepare_cb(GtkAssistant *assistant, GtkWidget *page, gpointer user_data) {
	leadrTcuCalibrationAssistant *cal_assistant = leadr_TCU_CALIBRATION_ASSISTANT(assistant);
	leadrTcuCalibrationAssistantPrivate *priv = cal_assistant->priv;
...
static void cancel_cb(GtkAssistant *assistant, gpointer user_data) {
	...
	leadr_tracking_control_unit_cancel(priv->device, priv->actual_dcu, &local_error);
	roccat_handle_error_dialog(priv->parent, _("Could not set TCU data"), &local_error);

	priv->tcu_value = leadr_TRACKING_CONTROL_UNIT_OFF;
...
static void apply_cb(GtkAssistant *assistant, gpointer user_data) {
	...
	leadr_tracking_control_unit_test(priv->device, priv->actual_dcu, priv->result, &local_error);
	...
	if (answer) {
		...
		leadr_tracking_control_unit_accept(priv->device, priv->actual_dcu, priv->result, &local_error);
		priv->tcu_value = leadr_TRACKING_CONTROL_UNIT_ON;
	} else {
		leadr_tracking_control_unit_cancel(priv->device, priv->actual_dcu, &local_error);
		priv->tcu_value = leadr_TRACKING_CONTROL_UNIT_OFF;
	}
...
GtkWidget *leadr_tcu_calibration_assistant_new(GtkWindow *parent, RoccatDevice *device, guint actual_dcu) {
	leadrTcuCalibrationAssistant *assistant;
	leadrTcuCalibrationAssistantPrivate *priv;

	assistant = leadr_TCU_CALIBRATION_ASSISTANT(g_object_new(leadr_TCU_CALIBRATION_ASSISTANT_TYPE,
			NULL));
...
static void leadr_tcu_calibration_assistant_init(leadrTcuCalibrationAssistant *assistant) {
	leadrTcuCalibrationAssistantPrivate *priv = leadr_TCU_CALIBRATION_ASSISTANT_GET_PRIVATE(assistant);
	assistant->priv = priv;
...
static void leadr_tcu_calibration_assistant_class_init(leadrTcuCalibrationAssistantClass *klass) {
	g_type_class_add_private(klass, sizeof(leadrTcuCalibrationAssistantPrivate));
